...
export default function MobileMenu3({
  data,
  handleMobileMenu,
  mobile_menu_active,
    ...
    <>
      {transitions((style, i) => (
        <a.div
          ...
          style={style}
          ref={ref}
          className={`fixed z-40  w-full top-4  h-[95%] max-w-sm right-4   rounded-[30px] bg-primary overflow-y-scroll  transition-all ease-in duration-[300ms]`}
        >
          ...
          <div className="flex justify-between flex-col h-[90%] pb-10">
                  ...
                  >
                    {item?.type == "SUB_MENU" && item.sub_menu ? (
                      <p
                        ...
                        className={`${color?.heading}  hover:text-highlight flex cursor-pointer gap-2 items-center text-xl  pb-1 duration-100 ease-linear`}
                      >
                        {item.name}
                        {item?.type == "SUB_MENU" && item.sub_menu ? (
                          <span
            ...
            {/* social links */}
            <div className="flex flex-col gap-5 items-center px-5 justify-center text-center ">
